...
function ApplyModal({ title, onClose, onSubmit, submitting, children, image }) {
          ...
          <form onSubmit={onSubmit} className="flex flex-col min-h-0 flex-1">
            ...
            <div className="sticky bottom-0 flex shrink-0 justify-end gap-3 border-t border-border bg-card px-5 py-4">
              ...
              <button
                type="submit"
                disabled={submitting}
                className="px-6 py-2 rounded-full bg-primary text-white text-sm font-semibold hover:bg-primary/90 transition-colors disabled:opacity-60"
              >
...
export default function Career() {
  ...
  useEffect(() => {
    api.getJobRoles().then(setJobRoles).catch(() => {});
    api.getInternRoles().then(setInternRoles).catch(() => {});
  }, []);
  ...
  const submitJob = async (e) => {
    ...
    setSubmitting(true);
    try {
      await api.createJobApplication({ ...jobForm, jobRole: jobModal.roleName });
      setJobModal(null);
      setJobForm(emptyJob);
      setJobErrors({});
  ...
  const submitIntern = async (e) => {
    ...
    setSubmitting(true);
    try {
      await api.createInternApplication(internForm);
  ...
  return (
    <Layout hideScrollToTop={Boolean(jobModal || internModal)} hideFooter={Boolean(internModal)}>
      ...
      {/* Job Apply Modal */}
      {jobModal && (
        <ApplyModal title={`Apply — ${jobModal.roleName}`} image="/assests/career.png" onClose={() => setJobModal(null)} onSubmit={submitJob} submitting={submitting}>
          <Field label="Full Name *" error={jobErrors.name}>
            <Input placeholder="Enter Your Name" value={jobForm.name} onChange={setJ("name")} className={FORM_INPUT_CLASS} />
          </Field>
          <Field label="Email *" error={jobErrors.email}>
            <Input type="email" placeholder="Enter Your Email" value={jobForm.email} onChange={setJ("email")} className={FORM_INPUT_CLASS} />
          </Field>
          <Field label="Phone Number *" error={jobErrors.phone}>
            <Input placeholder="Enter Your Number" value={jobForm.phone} onChange={setJ("phone")} className={FORM_INPUT_CLASS} />
          </Field>
          ...
          <Field label="Experience *" error={jobErrors.experience}>
            <Input placeholder="e.g. 2 years in Software development" value={jobForm.experience} onChange={setJ("experience")} className={FORM_INPUT_CLASS} />
          </Field>
      ...
      {/* Intern Apply Modal */}
      {internModal && (
        <ApplyModal title="Apply for Internship" image="/assests/career.png" onClose={() => setInternModal(false)} onSubmit={submitIntern} submitting={submitting}>
          <Field label="Full Name *" error={internErrors.name}>
            <Input placeholder="Enter Eour Name" value={internForm.name} onChange={setI("name")} className={FORM_INPUT_CLASS} />
          </Field>
          <Field label="Email *" error={internErrors.email}>
            <Input type="email" placeholder="Enter Your Email" value={internForm.email} onChange={setI("email")} className={FORM_INPUT_CLASS} />
          </Field>
          <Field label="Phone Number *" error={internErrors.phone}>
            <Input placeholder="Enter Your Name" value={internForm.phone} onChange={setI("phone")} className={FORM_INPUT_CLASS} />
          </Field>
