...
const GetStarted = () => {
  ...
  const handleSubmit = async () => {
    setSubmitting(true);
    try {
      await api.createProjectInquiry({
        ...formData,
              ...
              {step === 3 && (
                <motion.div key="step3" variants={stepVariants} initial="hidden" animate="visible" exit="exit">
                  ...
                  <div className="glass-card p-6 md:p-8 mb-6 max-w-lg mx-auto">
                    <div className="grid gap-4 mb-4">
                      <div>
                        ...
                        <Input className="h-10 text-sm" placeholder="Enter Your Name" value={formData.name} onChange={(e) => setFormData((p) => ({ ...p, name: e.target.value }))} />
                      </div>
                      <div>
                        <label className="block text-xs font-medium mb-1">Email Address *</label>
                        <Input className="h-10 text-sm" type="email" placeholder="Enter Your Email" value={formData.email} onChange={(e) => setFormData((p) => ({ ...p, email: e.target.value }))} />
                      </div>
                      <div>
                        <label className="block text-xs font-medium mb-1">Phone Number</label>
                        <Input className="h-10 text-sm" placeholder="Enter Your Number" value={formData.phone} onChange={(e) => setFormData((p) => ({ ...p, phone: e.target.value }))} />
                      </div>
                      <div>
                  ...
                  <div className="flex justify-between">
                    ...
                    <Button size="lg" onClick={handleSubmit} disabled={!canSubmit || submitting} className="glow-border hover-glow">
                      {submitting ? 'Submitting...' : 'Submit Request'} <ArrowRight className="ml-2 w-4 h-4" />
                    </Button>
                  </div>
                </motion.div>
              )}
